...
export const sortCol = (items, field) => {
  const compareString = (a, b) => {
    return a[field.key].toLowerCase() > b[field.key].toLowerCase()
      ? 1 * field.sort
      : -1 * field.sort;
  };
  const compareNumber = (a, b) => {
    return parseFloat(a[field.key]) > parseFloat(b[field.key])
      ? 1 * field.sort
      : -1 * field.sort;
  };
...
export const filterColumn = (field, itemsSvg) => {
  return itemsSvg.filter((element) => {
    return element[field.key]
      .toLowerCase()
      .includes(field.filter.toLowerCase());
  });
};
